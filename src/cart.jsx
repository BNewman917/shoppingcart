...
const Products = (props) => {
    ...
    const restockProducts = (url) => {
        const getIndex = (itemName) => {
            return items.map((item) => item.name).indexOf(itemName);
        };
        ...
        fetch(url, requestOptions)
            ...
            .then((result) => {
                ...
                let newStock = Object.keys(result.data).map((key, index) => {
                    let { name, country, cost, instock } =
                        result.data[index].attributes;
                    return { name, country, cost, instock };
                });

                let restock = newStock.map((item) => {
                    let itemIndex = getIndex(item.name);
                    return {
                        name: item.name,
                        country: item.country,
                        cost: item.cost,
                        instock: item.instock + items[itemIndex].instock,
                    };
                });
                setItems(restock);
            })
